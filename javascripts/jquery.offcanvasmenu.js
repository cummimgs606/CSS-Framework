...
(function() {
	...
	/*console.log("Function: offcanvasmenu");*/
  ...
  $.offCanvasMenu = function(options) {
    var actions, backfaceCss, baseCSS, body, background, container, cssSupport, head, inner, innerWrapper, menu,menuID, menuLeft, outer, outerWrapper, settings, transEndEventName, transformPosition, transformPrefix, trigger;
    settings = {
      direction: "left",
      coverage: "70%",
      menu: "#menu",
	...
	body = $(settings.menu).parent();
	body.attr('id', settings.menuID);
	
	$( "<div class='push-background'></div>" ).insertBefore(body);
    ...
    baseCSS = "<style type='text/css'>  " + outer + " {      left: 0;     /* overflow-x: hidden;*/     position: absolute;      top: 0;      width: 100%;    }    " + inner + " {      position: relative; " + backfaceCss + "}" + container + " " + settings.menu + " {      display : block;      height: 0;      left    : " + menuLeft + ";      margin  : 0;      overflow: hidden;      position: absolute;      top     : 0;      width   : " + settings.coverage + ";    }  </style>";
    head.append(baseCSS);
    actions = {
      ...
      on: function() {
		...
		*/
		...
		$(settings.menu).wrapAll('<div class="' + settings.classes.outer + '"/>');
		outerWrapper = $("." + settings.classes.outer);
		
		$(settings.menu).wrapAll('<div class="' + settings.classes.inner + '"/>');
		innerWrapper = $("." + settings.classes.inner);
      ...
      show: function() {
		...
		$(body).prev().css("visibility","visible");
      ...
      hide: function() {
		...
		$(body).prev().css("visibility","hidden");
